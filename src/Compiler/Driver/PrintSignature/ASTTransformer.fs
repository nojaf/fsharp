module internal rec PrintSignature.ASTTransformer
...
module RangePatterns =
    ...
    let (|StartEndRange|) (size: int) (range: range) =
        let o, c = mkStartEndRange size range
        o, range, c
...
let parseExpressionInSynBinding returnInfo expr =
    ...
    | Some(SynBindingReturnInfo(typeName = t1)), SynExpr.Typed(e, t2, _) when Range.equals t1.Range t2.Range -> e
    | _ -> expr
...
let mkParsedHashDirective (creationAide: CreationAide) (ParsedHashDirective(ident, args, range)) =
    let args =
        args
        |> List.map (function
            | ParsedHashDirectiveArgument.String(value, stringKind, range) -> mkConstString creationAide stringKind value range
...
let mkConstant (creationAide: CreationAide) c r : Constant =
    ...
    | SynConst.Measure(c, numberRange, measure, trivia) ->
        ...
        let unitOfMeasure =
            UnitOfMeasureNode(stn "<" trivia.LessRange, mkMeasure creationAide measure, stn ">" trivia.GreaterRange, uOfMRange)
...
let rec collectComputationExpressionStatements
    ...
    | SynExpr.LetOrUseBang(_, isUse, _, pat, expr, andBangs, body, StartRange 4 (mLeading, _m), { EqualsRange = Some mEq }) ->
        let letOrUseBang =
        ...
        let andBangs =
            ...
            |> List.map (fun (SynExprAndBang(_, _, _, ap, ae, StartRange 4 (mAnd, m), trivia)) ->
                ExprAndBang(stn "and!" mAnd, mkPat creationAide ap, stn "=" trivia.EqualsRange, mkExpr creationAide ae, m)
    ...
    | SynExpr.Sequential(_, _, e1, e2, _) ->
        let continuations
            : ((ComputationExpressionStatement list -> ComputationExpressionStatement list) -> ComputationExpressionStatement list) list =
            [
...
let (|ColonColonInfixApp|_|) =
    ...
    | SynExpr.App(
        isInfix = true
        funcExpr = SynExpr.LongIdent(longDotId = SynLongIdent([ operatorIdent ], [], [ Some(IdentTrivia.OriginalNotation "::") ]))
...
let (|InfixApp|_|) synExpr =
    ...
    | SynExpr.App(
        funcExpr = SynExpr.App(
            isInfix = true
            funcExpr = SynExpr.LongIdent(longDotId = SynLongIdent([ operatorIdent ], [], [ Some(IdentTrivia.OriginalNotation operator) ]))
            argExpr = e1)
...
let (|IndexWithoutDot|_|) expr =
    ...
    | SynExpr.App(ExprAtomicFlag.NonAtomic,
                  ...
                  _) when (isAdjacentTo identifierExpr.Range argExpr.Range) -> Some(identifierExpr, indexExpr)
    | _ -> None
        ...
        | SynExpr.App(ExprAtomicFlag.NonAtomic,
                      ...
                      SynExpr.App(
                          ...
                          funcExpr = SynExpr.LongIdent(
                              longDotId = SynLongIdent(id = [ operatorIdent ]; trivia = [ Some(IdentTrivia.OriginalNotation lhsOperator) ]))
                          argExpr = leftExpr),
                      rhs,
...
let rec (|ElIf|_|) =
    ...
    | SynExpr.IfThenElse(e1,
                         e2,
                         Some(ElIf((elifNode: Choice<SingleTextNode, range * range>, eshE1, eshThenKw, eshE2) :: es, elseInfo)),
                         _,
                         _,
                         _,
                         trivia) ->
...
let mkLinksFromFunctionName (mkLinkFromExpr: SynExpr -> LinkExpr) (functionName: SynExpr) : LinkExpr list =
    ...
    | SynExpr.TypeApp(SynExpr.LongIdent(longDotId = sli), lessRange, typeArgs, commaRanges, Some greaterRange, typeArgsRange, _) ->
        match sli.IdentsWithTrivia with
        | []
...
let (|ChainExpr|_|) (e: SynExpr) : LinkExpr list option =
    let rec visit (e: SynExpr) (continuation: LinkExpr list -> LinkExpr list) =
        ...
        | SynExpr.App(
            isInfix = false
            funcExpr = SynExpr.TypeApp(SynExpr.DotGet _ as funcExpr, lessRange, typeArgs, commaRanges, Some greaterRange, typeArgsRange, _)
            ...
            visit funcExpr (fun leftLinks ->
                let app =
                    ...
                    | Some(LinkExpr.Identifier identifier) ->
                        [
                            SynExpr.App(ExprAtomicFlag.Atomic, false, identifier, argExpr, unionRanges identifier.Range argExpr.Range)
                            |> LinkExpr.Expr
                        ]
                    | _ -> []
    ...
    | SynExpr.App(
        isInfix = false; funcExpr = SynExpr.DotGet _ | SynExpr.TypeApp(expr = SynExpr.DotGet _); argExpr = UnitExpr _ | ParenExpr _)
    | SynExpr.DotGet _
    | SynExpr.TypeApp(expr = SynExpr.DotGet _)
    | SynExpr.DotIndexedGet(objectExpr = SynExpr.App(funcExpr = SynExpr.DotGet _) | SynExpr.DotGet _) -> Some(visit e id)
    | _ -> None
...
let mkExpr (creationAide: CreationAide) (e: SynExpr) : Expr =
    ...
    | SynExpr.Record(baseInfo, copyInfo, recordFields, StartEndRange 1 (mOpen, _, mClose)) ->
        ...
        | None, None ->
            ...
            |> Expr.Record
    | SynExpr.AnonRecd(true, copyInfo, recordFields, (StartRange 6 (mStruct, _) & EndRange 2 (mClose, _)), { OpeningBraceRange = mOpen }) ->
        let fields =
            recordFields
            |> List.choose (function
                | sli, Some mEq, e ->
    ...
    | SynExpr.AnonRecd(false, copyInfo, recordFields, EndRange 2 (mClose, _), { OpeningBraceRange = mOpen }) ->
        ...
        ExprRecordNode(stn "{|" mOpen, Option.map (fst >> mkExpr creationAide) copyInfo, fields, stn "|}" mClose, exprRange)
        |> Expr.Record
    ...
    | SynExpr.For(_, _, ident, Some equalsRange, e1, isUp, e2, e3, StartRange 3 (mFor, _)) ->
        ...
        |> Expr.For
    | SynExpr.ForEach(_, _, SeqExprOnly true, _, pat, e1, SynExpr.YieldOrReturn((true, _), e2, _), StartRange 3 (mFor, _)) ->
        ExprForEachNode(stn "for" mFor, mkPat creationAide pat, mkExpr creationAide e1, true, mkExpr creationAide e2, exprRange)
        |> Expr.ForEach
    | SynExpr.ForEach(_, _, SeqExprOnly isArrow, _, pat, e1, e2, StartRange 3 (mFor, _)) ->
        ExprForEachNode(stn "for" mFor, mkPat creationAide pat, mkExpr creationAide e1, isArrow, mkExpr creationAide e2, exprRange)
        |> Expr.ForEach
    | SynExpr.App(ExprAtomicFlag.NonAtomic,
                  false,
    ...
    | SynExpr.MatchLambda(_, mFunction, cs, _, _) -> mkMatchLambda creationAide mFunction cs exprRange |> Expr.MatchLambda
    ...
    | SynExpr.App(_,
                  false,
                  SynExpr.LongIdent(_, SynLongIdent([ ident ], [], [ Some(IdentTrivia.OriginalNotation operatorName) ]), _, _),
                  e2,
                  _) when PrettyNaming.IsValidPrefixOperatorDefinitionName(PrettyNaming.ConvertValLogicalNameToDisplayNameCore ident.idText) ->
    ...
    | SynExpr.App(ExprAtomicFlag.NonAtomic,
                  ...
                  SynExpr.App(ExprAtomicFlag.NonAtomic,
                              ...
                              _),
                  argExpr,
                  _) when (isAdjacentTo identifierExpr.Range indexArgExpr.Range) ->
        ExprNestedIndexWithoutDotNode(
            mkExpr creationAide identifierExpr,
            mkExpr creationAide indexExpr,
            mkExpr creationAide argExpr,
    ...
    | ElIf([ elifKw, ifExpr, thenKw, thenExpr ], Some(elseKw, elseExpr)) ->
        ...
        ExprIfThenElseNode(ifKwNode, ifExprNode, thenKw, mkExpr creationAide thenExpr, elseKw, mkExpr creationAide elseExpr, exprRange)
        |> Expr.IfThenElse
    ...
    | SynExpr.LongIdent(isOpt, synLongIdent, _, m) -> ExprOptVarNode(isOpt, mkSynLongIdent synLongIdent, m) |> Expr.OptVar
    ...
    | SynExpr.DotIndexedSet(objectExpr, indexExpr, valueExpr, _, _, _) ->
        ExprDotIndexedSetNode(mkExpr creationAide objectExpr, mkExpr creationAide indexExpr, mkExpr creationAide valueExpr, exprRange)
        |> Expr.DotIndexedSet
    | SynExpr.NamedIndexedPropertySet(synLongIdent, e1, e2, _) ->
        ExprNamedIndexedPropertySetNode(mkSynLongIdent synLongIdent, mkExpr creationAide e1, mkExpr creationAide e2, exprRange)
    ...
    | SynExpr.LibraryOnlyStaticOptimization(constraints, e, optExpr, _) ->
        let constraints =
            ...
            |> List.map (function
                ...
                | SynStaticOptimizationConstraint.WhenTyparIsStruct(t, _) -> mkSynTypar t |> StaticOptimizationConstraint.WhenTyparIsStruct)

        ExprLibraryOnlyStaticOptimizationNode(mkExpr creationAide optExpr, constraints, mkExpr creationAide e, exprRange)
    ...
    | SynExpr.IndexRange(Some(SynExpr.IndexRange(Some(ConstNumberExpr(c1, mC1)), mDots1, Some(ConstNumberExpr(c2, mC2)), _, _, _)),
                         mDots2,
    ...
    | SynExpr.IndexRange(expr1 = e1; opm = mDots; expr2 = e2) ->
        ...
        ExprIndexRangeNode(Option.map (mkExpr creationAide) e1, dotsNode, Option.map (mkExpr creationAide) e2, exprRange)
        |> Expr.IndexRange
...
let (|PatParameter|_|) (p: SynPat) =
    ...
    | SynPat.Attrib(pat = SynPat.Typed(pat = pat; targetType = t); attributes = attributes) -> Some(attributes, pat, Some t)
...
let mkPat (creationAide: CreationAide) (p: SynPat) =
    ...
    | PatParameter(ats, pat, t) ->
        PatParameterNode(mkAttributes creationAide ats, mkPat creationAide pat, Option.map (mkType creationAide) t, patternRange)
        |> Pattern.Parameter
    | SynPat.Or(p1, p2, _, trivia) ->
        PatLeftMiddleRight(mkPat creationAide p1, Choice1Of2(stn "|" trivia.BarRange), mkPat creationAide p2, patternRange)
        |> Pattern.Or
    ...
    | SynPat.Named(accessibility = ao; ident = ident) -> PatNamedNode(mkSynAccess ao, mkSynIdent ident, patternRange) |> Pattern.Named
    | SynPat.As(p1, p2, _) ->
        PatLeftMiddleRight(mkPat creationAide p1, Choice2Of2 "as", mkPat creationAide p2, patternRange)
        |> Pattern.As
    | SynPat.ListCons(p1, p2, _, trivia) ->
        PatLeftMiddleRight(mkPat creationAide p1, Choice1Of2(stn "::" trivia.ColonColonRange), mkPat creationAide p2, patternRange)
        |> Pattern.ListCons
    ...
    | SynPat.LongIdent(synLongIdent, _, vtdo, SynArgPats.Pats pats, ao, _) ->
        ...
        PatLongIdentNode(mkSynAccess ao, mkSynLongIdent synLongIdent, typarDecls, List.map (mkPat creationAide) pats, patternRange)
        |> Pattern.LongIdent
    ...
    | SynPat.IsInst(t, StartRange 2 (tokenRange, _)) ->
        ...
        |> Pattern.IsInst
    | SynPat.QuoteExpr(SynExpr.Quote(_, isRaw, e, _, _), _) -> mkExprQuote creationAide isRaw e patternRange |> Pattern.QuoteExpr
...
let (|OperatorWithStar|_|) (si: SynIdent) =
    match si with
    | SynIdent(ident, Some(ParenStarSynIdent(_, text, _))) -> Some(IdentifierOrDot.Ident(stn $"( %s{text} )" ident.idRange))
    | _ -> None

let mkBinding (creationAide: CreationAide) (SynBinding(_, _, _, isMutable, attributes, xmlDoc, _, pat, returnInfo, expr, _, _, trivia)) =
    ...
    let ao, functionName, genericParameters, parameters =
        ...
        | SynPat.LongIdent(accessibility = ao; longDotId = lid; typarDecls = typarDecls; argPats = SynArgPats.Pats ps) ->
            ao, Choice1Of2(mkFunctionName lid), mkSynValTyparDecls creationAide typarDecls, List.map (mkPat creationAide) ps
...
let mkExternBinding
    ...
    let identifier, openNode, parameters, closeNode =
        match pat with
        | SynPat.LongIdent(longDotId = longDotId; argPats = SynArgPats.Pats [ SynPat.Tuple(_, ps, _, StartEndRange 1 (mOpen, _, mClose)) ]) ->
...
let mkModuleDecl (creationAide: CreationAide) (decl: SynModuleDecl) =
    ...
    | SynModuleDecl.Exception(SynExceptionDefn(SynExceptionDefnRepr(attrs, caseName, _, xmlDoc, vis, _), withKeyword, ms, _), _) ->
        ExceptionDefnNode(
            mkXmlDoc xmlDoc,
    ...
    | SynModuleDecl.Let(_,
                        ...
                        _) -> mkExternBinding creationAide binding |> ModuleDecl.ExternBinding
    | SynModuleDecl.Let(bindings = [ singleBinding ]) -> mkBinding creationAide singleBinding |> ModuleDecl.TopLevelBinding
...
let mkSynRationalConst (creationAide: CreationAide) rc =
    let rec visit rc =
        ...
        | SynRationalConst.Paren(SynRationalConst.Rational(numerator, numeratorRange, divRange, denominator, denominatorRange, _), range) ->
            let openingParen =
                let r =
...
let mkSynTypar (SynTypar(ident, req, _)) =
    let range =
        mkRange ident.idRange.FileName (Position.mkPos ident.idRange.StartLine (ident.idRange.StartColumn - 1)) ident.idRange.End
...
let mkType (creationAide: CreationAide) (t: SynType) : Type =
    ...
    | SynType.App(t, Some mLt, args, _commaRanges, Some mGt, false, _) ->
        TypeAppPrefixNode(mkType creationAide t, None, stn "<" mLt, List.map (mkType creationAide) args, stn ">" mGt, typeRange)
        |> Type.AppPrefix
...
let mkSynLeadingKeyword (lk: SynLeadingKeyword) =
    ...
    | SynLeadingKeyword.AbstractMember(abstractRange, memberRange) -> mtn [ "abstract", abstractRange; "member", memberRange ]
    ...
    | SynLeadingKeyword.StaticAbstract(staticRange, abstractRange) -> mtn [ "static", staticRange; "abstract", abstractRange ]
    ...
    | SynLeadingKeyword.StaticLetRec(staticRange, letRange, recRange) -> mtn [ "static", staticRange; "let", letRange; "rec", recRange ]
...
let mkSynUnionCase (creationAide: CreationAide) (SynUnionCase(attributes, ident, caseType, xmlDoc, _vis, m, trivia)) : UnionCaseNode =
    let fullRange =
        if not xmlDoc.IsEmpty then
            m
        else
...
let mkSynSimplePat creationAide (pat: SynSimplePat) =
    match pat with
    | SynSimplePat.Attrib(SynSimplePat.Typed(SynSimplePat.Id(ident = ident; isOptional = isOptional), t, _), attributes, m) ->
        Some(SimplePatNode(mkAttributes creationAide attributes, isOptional, mkIdent ident, Some(mkType creationAide t), m))
...
let mkImplicitCtor creationAide vis (attrs: SynAttributeList list) pats (self: (range * Ident) option) (xmlDoc: PreXmlDoc) =
    ...
    ImplicitConstructorNode(mkXmlDoc xmlDoc, mkAttributes creationAide attrs, mkSynAccess vis, openNode, pats, closeNode, asSelfNode, range)

let mkTypeDefn (creationAide: CreationAide) (SynTypeDefn(typeInfo, typeRepr, members, implicitConstructor, range, trivia)) : TypeDefn =
    let typeNameNode =
        match typeInfo with
    ...
    | SynTypeDefnRepr.Simple(simpleRepr = SynTypeDefnSimpleRepr.Record(ao, fs, StartEndRange 1 (openingBrace, _, closingBrace))) ->
        let fields = List.map (mkSynField creationAide) fs

        TypeDefnRecordNode(typeNameNode, mkSynAccess ao, stn "{" openingBrace, fields, stn "}" closingBrace, members, typeDefnRange)
        |> TypeDefn.Record
    ...
    | SynTypeDefnRepr.ObjectModel(
        kind = SynTypeDefnKind.Class | SynTypeDefnKind.Interface | SynTypeDefnKind.Struct as tdk; members = objectMembers; range = range) ->
        let kindNode =
    ...
    | SynTypeDefnRepr.ObjectModel(kind = SynTypeDefnKind.Delegate(signature = TFuns(ts, rt) as st); range = StartRange 8 (mDelegate, _)) ->
...
let mkMemberDefn (creationAide: CreationAide) (md: SynMemberDefn) =
    ...
    | SynMemberDefn.GetSetMember(Some(SynBinding(_,
                                                 ...
                                                 SynPat.LongIdent(lid,
                                                                  ...
                                                                  SynArgPats.Pats [ SynPat.Paren(
                                                                                        pat = SynPat.Const(constant = SynConst.Unit)) ],
                                                                  ao,
                                                                  mPat),
                                                 ri,
                                                 e,
        ...
        mkBinding creationAide (SynBinding(None, kind, isInline, isMutable, ats, px, valData, pat, ri, e, bindingRange, dp, trivia))
        |> MemberDefn.Member
    | SynMemberDefn.Member(
        memberDefn = SynBinding(
            ...
            headPat = SynPat.LongIdent(
                longDotId = SynLongIdent(id = [ newIdent ]); argPats = SynArgPats.Pats [ SynPat.Paren _ as pat ]; accessibility = ao)
            expr = expr
    ...
    | SynMemberDefn.LetBindings(bindings = [ SynBinding(kind = SynBindingKind.Do; expr = expr; trivia = trivia) ]) ->
        ...
        ExprSingleNode(stn leadingKw trivia.LeadingKeyword.Range, true, false, mkExpr creationAide expr, memberDefinitionRange)
        |> MemberDefn.DoExpr
    ...
    | SynMemberDefn.Interface(t, mWith, mdsOpt, _) ->
        ...
        MemberDefnInterfaceNode(interfaceNode, mkType creationAide t, Option.map (stn "with") mWith, members, memberDefinitionRange)
    ...
    | SynMemberDefn.GetSetMember(Some(SynBinding(
        ...
        let firstBinding =
            ...
            | SynBinding(headPat = SynPat.LongIdent(accessibility = Some vis)) when rangeBeforePos memberName.Range vis.Range.Start ->
...
let mkVal (creationAide: CreationAide) (SynValSig(ats, synIdent, vtd, t, _vi, _isInline, isMutable, px, ao, eo, range, trivia)) : ValNode =
    let lk =
        match trivia.LeadingKeyword with
...
let mkMemberSig (creationAide: CreationAide) (ms: SynMemberSig) =
    ...
    | SynMemberSig.Member(vs, _, _, memberTrivia) ->
        ...
        MemberDefnSigMemberNode(mkVal creationAide vs, mkWithGetSet trivia.WithKeyword memberTrivia.GetSetKeywords, memberSigRange)
...
let rec mkModuleDecls (creationAide: CreationAide) (decls: SynModuleDecl list) (finalContinuation: ModuleDecl list -> ModuleDecl list) =
    match decls with
    | [] -> finalContinuation []
    | OpenL(xs, ys) ->
        let openListNode =
    ...
    | head :: tail -> mkModuleDecls creationAide tail (fun nodes -> mkModuleDecl creationAide head :: nodes |> finalContinuation)
...
let mkModuleOrNamespace
    ...
    let leadingKeyword =
        match trivia.LeadingKeyword with
        | SynModuleOrNamespaceLeadingKeyword.Module mModule -> Some(MultipleTextsNode([ stn "module" mModule ], mModule))
...
let mkImplFile (creationAide: CreationAide) (ParsedImplFileInput(hashDirectives = hashDirectives; contents = contents)) (m: range) =
...
let mkModuleSigDecl (creationAide: CreationAide) (decl: SynModuleSigDecl) =
    ...
    match decl with
    | SynModuleSigDecl.Exception(SynExceptionSig(SynExceptionDefnRepr(attrs, caseName, _, xmlDoc, vis, _), withKeyword, ms, _), _) ->
        ExceptionDefnNode(
            mkXmlDoc xmlDoc,
            mkAttributes creationAide attrs,
            mkSynAccess vis,
...
let mkTypeDefnSig (creationAide: CreationAide) (SynTypeDefnSig(typeInfo, typeRepr, members, range, trivia)) : TypeDefn =
    ...
    | SynTypeDefnSigRepr.Simple(repr = SynTypeDefnSimpleRepr.Record(ao, fs, StartEndRange 1 (openingBrace, _, closingBrace))) ->
        let fields = List.map (mkSynField creationAide) fs
    ...
    | SynTypeDefnSigRepr.ObjectModel(
        kind = SynTypeDefnKind.Class | SynTypeDefnKind.Interface | SynTypeDefnKind.Struct as tdk; memberSigs = objectMembers; range = range) ->
    ...
    | SynTypeDefnSigRepr.ObjectModel(kind = SynTypeDefnKind.Delegate(signature = TFuns(ts, rt) as st); range = StartRange 8 (mDelegate, _)) ->
        let typeList = mkTypeList creationAide ts rt st.Range
...
let rec mkModuleSigDecls
    ...
    | head :: tail -> mkModuleSigDecls creationAide tail (fun nodes -> mkModuleSigDecl creationAide head :: nodes |> finalContinuation)
...
let mkSigFile (creationAide: CreationAide) (ParsedSigFileInput(hashDirectives = hashDirectives; contents = contents)) (m: range) =
    let phds = List.map (mkParsedHashDirective creationAide) hashDirectives
    let mds = List.map (mkModuleOrNamespaceSig creationAide) contents
    Oak(phds, mds, m)

let includeTrivia (baseRange: range) (comments: CommentTrivia list) (conditionDirectives: ConditionalDirectiveTrivia list) : range =
    let ranges =
        [
            yield!
                List.map
...
let mkFullTreeRange (ParsedSigFileInput(hashDirectives = directives; contents = modules; trivia = trivia)) =
    let startPos =
...
let mkOak (parsedSigFileInput: ParsedSigFileInput) =
    let creationAide = { SourceText = None }
    let fullRange = mkFullTreeRange parsedSigFileInput
    mkSigFile creationAide parsedSigFileInput fullRange
