module internal rec PrintSignature.CodePrinter
...
let genExpr (e: Expr) =
        ...
        let genStatements =
            ...
            |> List.map (function
                ...
                | ComputationExpressionStatement.AndBangStatement node ->
                    ...
                    ColMultilineItem(expr, sepNlnUnlessContentBefore node)
                | ComputationExpressionStatement.OtherStatement e -> ColMultilineItem(genExpr e, sepNlnUnlessContentBefore (Expr.Node e)))
            |> colWithNlnWhenItemIsMultilineUsingConfig
            |> genNode node

        match node.Statements with
        | [ ComputationExpressionStatement.LetOrUseStatement letOrUseNode; ComputationExpressionStatement.OtherStatement inExpr ] when
            letOrUseNode.In.IsSome
            ->
            let short =
    ...
    | Expr.ParenLambda node ->
        genSingleTextNode node.OpeningParen
        +> leadingExpressionIsMultiline (genLambdaWithParen node.Lambda +> sepNlnWhenWriteBeforeNewlineNotEmpty) (fun isMultiline ctx ->
            onlyIf
                (isMultiline
    ...
    | Expr.SameInfixApps node ->
        ...
        fun ctx -> genNode node (atCurrentColumn (isShortExpression ctx.Config.MaxInfixOperatorExpression shortExpr multilineExpr)) ctx
            ...
            fun ctx ->
                genNode
                    ...
                    (isShortExpression
                        ...
                        (ifElse (noBreakInfixOps.Contains(node.Operator.Text)) (genOnelinerInfixExpr node) (genMultilineInfixExpr node)))
                    ctx
    ...
    | Expr.Chain node ->
        let genLink (isLastLink: bool) (link: ChainLink) =
            ...
            | ChainLink.AppUnit appUnitNode ->
                ...
                +> onlyIf
                    isLastLink
                    (sepSpaceBeforeParenInFuncInvocation appUnitNode.FunctionName (Expr.Constant(Constant.Unit appUnitNode.Unit)))
                +> genUnit appUnitNode.Unit
                |> genNode appUnitNode
            | ChainLink.AppParen appParen ->
                let short =
                    genExpr appParen.FunctionName
                    +> onlyIf isLastLink (sepSpaceBeforeParenInFuncInvocation appParen.FunctionName (Expr.Paren appParen.Paren))
    ...
    | Expr.AppWithLambda node ->
        let sepSpaceAfterFunctionName =
            ...
            | [] ->
                ...
                let parenExpr =
                    mkExprParenNode node.OpeningParen (Expr.Null(SingleTextNode("", Range.Zero))) node.ClosingParen Range.Zero
    ...
    | Expr.App node ->
        fun ctx ->
            match node with
            | EndsWithDualListApp ctx.Config (sequentialArgs: Expr list, firstList: ExprArrayOrListNode, lastList: ExprArrayOrListNode) ->
    ...
    | Expr.IfThenElif node ->
        ...
        let shortExpr =
            col sepNln node.Branches (fun (node: ExprIfThenNode) ->
                ...
                +> genExpr node.ThenExpr
                |> genNode node)
            +> optSingle (fun (elseNode, elseExpr) -> sepNln +> genSingleTextNode elseNode +> sepSpace +> genExpr elseExpr) node.Else
    ...
    | Expr.DotIndexedGet node ->
        ...
        | Expr.App appNode ->
            match appNode.Arguments with
            | [ Expr.Constant(Constant.Unit _) as ux ] -> genDotIndexedGetWithApp (genExpr appNode.FunctionExpr) ux appNode
            | _ -> genDotIndexedGet
        | Expr.AppSingleParenArg appNode -> genDotIndexedGetWithApp (genExpr appNode.FunctionExpr) appNode.ArgExpr appNode
    ...
    | Expr.DotIndexedSet node ->
        ...
        | Expr.App appNode ->
            match appNode.Arguments with
            | [ Expr.Constant(Constant.Unit _) as ux ] -> genDotIndexedSetWithApp (genExpr appNode.FunctionExpr) ux appNode
            | _ -> genDotIndexedSet

        | Expr.AppSingleParenArg appNode -> genDotIndexedSetWithApp (genExpr appNode.FunctionExpr) appNode.ArgExpr appNode
...
let genMultilineRecordFieldsExpr (genRecordField: RecordFieldNode -> Context -> Context) (node: ExprRecordBaseNode) : Context -> Context =
...
let genAppLongIdentAndSingleParenArgExpr (addSpace: Context -> Context) (node: ExprAppLongIdentAndSingleParenArgNode) =
    ...
    let long =
        ...
        ifElseCtx (futureNlnCheck shortLids) (genFunctionNameWithMultilineLids args node.FunctionName) (shortLids +> args)
...
let genControlExpressionStartCore (startKeyword: Choice<SingleTextNode, IfKeywordNode>) (innerExpr: Expr) (endKeyword: SingleTextNode) =
    let enterStart =
...
let genPat (p: Pattern) =
    ...
    | Pattern.LongIdent node ->
        ...
        let genParameters =
            ...
            | [] -> sepNone
            | [ Pattern.Paren _ | Pattern.Unit _ as parameter ] -> addSpaceBeforeParenInPattern node.Identifier +> genPat parameter
            ...
            let genSignature =
                ...
                let long (ctx: Context) =
                    ...
                    (onlyIf hasTriviaAfterLeadingKeyword indent
                     ...
                     +> leadingExpressionIsMultiline (genReturnType (not hasSingleTupledArg || alternativeSyntax)) (fun isMultiline ->
...
let genTypeDefn (td: TypeDefn) =
        ...
        +> leadingExpressionIsMultiline
            ...
            (fun isMulti ctx ->
                if isMulti && ctx.Config.AlternativeLongMemberDefinitions then
                    ...
                    (sepSpaceOrIndentAndNlnIfExpressionExceedsPageWidth (optSingle genSingleTextNode typeName.EqualsToken)) ctx)
        |> genNode typeName
    ...
    | TypeDefn.Union node ->
        ...
        let unionCases (ctx: Context) =
            ...
            | [ singleCase ] when not hasMembers ->
                ...
                let genCase hasVerticalBar =
                    opt sepSpace node.Accessibility (fun vis -> genSingleTextNode vis +> onlyIfNot singleCase.XmlDoc.IsNone sepNln)
                    +> genUnionCase hasVerticalBar singleCase

                expressionFitsOnRestOfLine (sepSpace +> genCase hasVerticalBar) (indentSepNlnUnindent (genCase true)) ctx
            | xs ->
                indentSepNlnUnindent
        ...
        header
        +> unionCases
        +> onlyIf hasMembers (indentSepNlnUnindent (sepNlnBetweenTypeAndMembers typeDefnNode +> genMemberDefnList members))
        |> genNode node
