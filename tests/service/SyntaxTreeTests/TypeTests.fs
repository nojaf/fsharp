                                ...
                                SynTypeDefn.SynTypeDefn(typeRepr =
                                    SynTypeDefnRepr.Simple(simpleRepr =
                                        SynTypeDefnSimpleRepr.Enum(cases =
                                            [
                                                SynEnumCase.SynEnumCase(valueRange = r)
                                            ]
                                        )
                                    )
                                )
                            ...
                            [
                                SynTypeDefn.SynTypeDefn(typeRepr =
                                    SynTypeDefnRepr.Simple(simpleRepr =
                                        SynTypeDefnSimpleRepr.Enum(cases =
                                            [
                                                SynEnumCase.SynEnumCase(valueRange = r1); SynEnumCase.SynEnumCase(valueRange = r2)
                                            ]
                                        )
                                    )
                                )
                                    ...
                                    Some(
                                        SynBindingReturnInfo(typeName =
                                            SynType.Tuple(
                                                true,
                                                [
                                                    SynTupleTypeSegment.Type _; SynTupleTypeSegment.Star _; SynTupleTypeSegment.Type _
                                                ],
                                                mTuple
                                            )
                                        )
                                    ...
                                    SynTypeDefnRepr.ObjectModel(kind =
                                        SynTypeDefnKind.Delegate(signature =
                                            SynType.Fun(
                                                ...
                                                returnType =
                                                    SynType.Fun(
                                                        argType = SynType.SignatureParameter(id = Some c); returnType = SynType.LongIdent _
                                                    )
                                            )
                                        )
                                    )
                                                    ...
                                                    SynType.Fun(
                                                        argType =
                                                            SynType.SignatureParameter(
                                                                [ { Attributes = [ _; _ ] } ], true, Some a, SynType.LongIdent _, m
                                                            )
