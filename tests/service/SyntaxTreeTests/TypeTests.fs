                    ...
                    SynTypeDefn.SynTypeDefn(typeRepr =
                        SynTypeDefnRepr.Simple(simpleRepr =
                            SynTypeDefnSimpleRepr.Enum(cases = [
                                SynEnumCase.SynEnumCase(valueRange = r1)
...
type Bar =
    ...
    | ParsedInput.ImplFile(
        ParsedImplFileInput(contents = [
            SynModuleOrNamespace.SynModuleOrNamespace(decls = [ SynModuleDecl.Types(typeDefns = [ t ]) as types ])
        ])
      ) ->
...
and [<CustomEquality ; NoComparison>] Bar<'context, 'a> =
    ...
    | ParsedInput.ImplFile(
        ParsedImplFileInput(contents = [
            SynModuleOrNamespace.SynModuleOrNamespace(decls = [ SynModuleDecl.Types(typeDefns = [ t1; t2 ]) as types ])
        ])
      ) ->
...
"""
    ...
    | ParsedInput.ImplFile(
        ParsedImplFileInput(contents = [
            SynModuleOrNamespace.SynModuleOrNamespace(decls = [
                SynModuleDecl.Types(typeDefns = [
                    SynTypeDefn(typeRepr = SynTypeDefnRepr.ObjectModel(kind = SynTypeDefnKind.Augmentation mWithKeyword))
                ])
            ])
        ])
...
"""
    ...
    | ParsedInput.ImplFile(
        ParsedImplFileInput(contents = [
            SynModuleOrNamespace.SynModuleOrNamespace(decls = [
                SynModuleDecl.Types(typeDefns = [
                    SynTypeDefn(trivia = { LeadingKeyword = SynTypeDefnLeadingKeyword.Type mType })
...
"""
    ...
    | ParsedInput.ImplFile(
        ParsedImplFileInput(contents = [
            SynModuleOrNamespace.SynModuleOrNamespace(decls = [
                SynModuleDecl.Types(typeDefns = [ SynTypeDefn(trivia = { LeadingKeyword = SynTypeDefnLeadingKeyword.Type mType }) ])
            ])
        ])
      ) -> assertRange (4, 0) (4, 4) mType
    | _ -> Assert.Fail "Could not get valid AST"
        ...
        ParsedImplFileInput(contents = [
            SynModuleOrNamespace.SynModuleOrNamespace(decls = [
                SynModuleDecl.Types(typeDefns = [
                    SynTypeDefn(typeRepr =
                        SynTypeDefnRepr.Simple(simpleRepr =
                            SynTypeDefnSimpleRepr.TypeAbbrev(rhsType = SynType.Fun(trivia = { ArrowRange = mArrow }))
                        )
                    )
                ])
            ])
                    ...
                    SynBinding(returnInfo =
                        Some(
                            SynBindingReturnInfo(typeName =
                                SynType.Tuple(
                                    true, [ SynTupleTypeSegment.Type _; SynTupleTypeSegment.Star _; SynTupleTypeSegment.Type _ ], mTuple
                                )
                            )
                        )
                    )
    ...
    | ParsedInput.ImplFile(
        ParsedImplFileInput(contents = [
            SynModuleOrNamespace.SynModuleOrNamespace(decls = [
                SynModuleDecl.Types(typeDefns = [
                    SynTypeDefn(typeRepr =
                        SynTypeDefnRepr.ObjectModel(kind =
                            SynTypeDefnKind.Delegate(signature =
                                SynType.Fun(
                                    argType =
                                        SynType.Tuple(path = [
                                    ...
                                    returnType =
                                        SynType.Fun(argType = SynType.SignatureParameter(id = Some c); returnType = SynType.LongIdent _)
                                )
                            )
                        )
                    )
                        ...
                        SynTypeDefnRepr.ObjectModel(members = [
                            SynMemberDefn.AbstractSlot(slotSig =
                                SynValSig(synType =
                                    SynType.Fun(
                                        argType =
                                            SynType.SignatureParameter([ { Attributes = [ _; _ ] } ], true, Some a, SynType.LongIdent _, m)
